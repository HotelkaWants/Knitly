...
class FirebaseAuthenticationHelper{
    ...
    companion object{
        ...
        fun createUserWithEmailAndPassword(
            ...
            return auth.createUserWithEmailAndPassword(email, password).addOnCompleteListener{ task ->
                if (task.isSuccessful){
                    FirebaseDB.checkExistUser(task.result.user?.uid.toString(), onExist = {UpdateUI(task.result.user!!, activity)}){
        ...
        suspend fun signOutGoogle(credentialManager: CredentialManager, startActivity: ComponentActivity){
            try {
